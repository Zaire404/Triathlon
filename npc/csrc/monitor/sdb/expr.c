  ...
  while (e[position] != '\0') {
    ...
    for (i = 0; i < NR_REGEX; i++) {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 &&
        ...
        printf("match rules[%d] = \"%s\" at position %d with len %d: %.*s", i,
...
bool check_parentheses(int l, int r) {
  ...
  check = (lp == 0) & (tokens[l].type == TK_left) & (tokens[r].type == TK_righ)
              ? check
              : 0;
  return check;
}
...
unsigned int eval(int l, int r) {
  ...
  if (l == r) {
    ...
    if (tokens[l].type == TK_NUMBER16)
      number = strtol(tokens[l].str, NULL, 16);
    else if (tokens[l].type == TK_NUMBER10)
      number = strtol(tokens[l].str, NULL, 10);
    else if (tokens[l].type == TK_REG)
      number = isa_reg_str2val(tokens[l].str + 1, NULL);
    return number;
...
word_t expr(char *e, bool *success) {
  ...
  for (int i = 0; i < nr_token; i++) {
    if (tokens[i].type == TK_MUL &&
        (i == 0 || ((tokens[i - 1].type != TK_righ) &&
                    (tokens[i - 1].type != TK_NUMBER10) &&
                    (tokens[i - 1].type != TK_NUMBER16)))) {
      tokens[i].type = TK_De;
      printf("%d\n", i);
    }
  }
